...
// Productos
const productos = [];
const URLproductos = "http://localhost:8000/api/products/";
...
function retornarProductCard({ imagen, nombre, precio, id }) {
    return `
            <div class="product-card">
                <img src="../../static/assets/${imagen}" alt="Nike Dunk Low">
...
function ActivarClickBtnCarrito(btnCarrito) {
    btnCarrito.addEventListener("click", () => {
        localStorage.setItem("carrito", JSON.stringify(carrito));
        carrito.length > 0 ? (location.href =  "/checkout") : mostrarMensajeCarrito("Primero debe cargar al menos un producto en su carrito", "error", "Error!");
    });
}
// #endregion
